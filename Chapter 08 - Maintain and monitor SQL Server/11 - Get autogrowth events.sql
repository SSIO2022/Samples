...
SELECT [g].[DatabaseName] AS [DB]
    ...
    CONVERT(DECIMAL(19, 2), [g].[IntegerData] * 8 / 1024.0) AS [EventGrowth_MB]
    , [g].[StartTime]
    , --Time of the autogrowth event 
    -- Length of time (in seconds) necessary to extend the file. 
    CONVERT(DECIMAL(19, 2), [g].[Duration] / 1000.0 / 1000.0) AS [EventDuration_s]
    , CASE 
        WHEN [mf].[is_percent_growth] = 1
            THEN CONVERT(CHAR(2), [mf].[growth]) + ' % '
        ELSE CONVERT(VARCHAR(30), [mf].[growth] * 8.0 / 1024.0) + 'MB'
        END AS [Current_Auto_Growth_Set]
    , CONVERT(DECIMAL(19, 2), [mf].[size] * 8.0 / 1024.0) AS [Current_File_Size_MB]
