...
/*
For example, consider the following script. In it, we will create a new database owned by sa, then one by a new principal named TestDbManager. The only thing this login will not be able to do with the database is change the owner, but it can drop the database, and make important setting changes.
*/
...
/*
Now, still as the sysadmin enabled login you are using to administer your test instance, create a database and make it owned by the sa built in login.
*/
...
--I suggest you always test that you are in the right context when you are writing test scripts. Too often you think it worked, but you were in the wrong security context.
...
/*
This command fails with the following error, indicating that it does not have permissions to change the owner. The login will be able to alter authorization to itself, or any other account they can access.
*/
...
/*
Note that being in the ##MS_DatabaseManager## role does not confer any rights in the databases created, but if the role member make themselves the owner of the database, they will have access. You can determine this using this query:
*/
...
--But each of the next three statements will fail, the first two will question if the role exists.
...
/*
This is an important distinction between the ##MS_LoginManager## role and the securityadmin role. It is allowed to grant new rights that it doesn’t have (though not membership to a role it is not a member of). 
If you want your new login to be able to grant rights, you will need to give it either give the account CONTROL rights over a resource, or use the WITH GRANT OPTION on your GRANT statements, which will allow the account to GRANT the same rights to other principals. Go back to your sysadmin level account.
*/

REVERT;

--Then add the login to the ##MS_DatabaseConnector##, and you will see that you can now add your WhatCanIDo account to it as well.
