...
/*
In the previous section, we created the login for [Domain\Fred], so the CREATE USER statement referenced that login. However, a user could be created for that login regardless of the existence of the explicit login principal.
*/
...
/*
The db_owner role does not specifically confer the CONTROL DATABASE permission to it’s members, but is equivalent in terms of what it is allowed to do. Different from the sysadmin server role, the db_owner role does not bypass DENY permissions. For example:
*/
...
/*
(While impersonating dbo, the user can easily revoke any denied rights, so it is important to audit users with elevated rights if they have access to any sensitive data.)
*/
