...
/*  Assuming you are using the same name for user and login, you can use the 
    following script in the restored database to generate an ALTER USER script 
    for your SQL Login based database principals to map them to server logins. */
...
/*  In the case where you are transferring all of the logins for a server, 
    you can actually prevent orphaned SIDS. You can re-create SQL 
    Server–authenticated logins on multiple servers, each having the same 
    SID. This is not possible using SSMS user interface; instead, you must 
    accomplish this by using the CREATE LOGIN command, as shown here (or 
    using tools we will look at later in this chapter): */
CREATE LOGIN [Kirby] WITH PASSWORD=N'<strongpassword>', SID = 0x5931F5B9C157464EA244B9D381DC5CCC;

/*  Common security administration tasks\How to migrate SQL Server 
    logins\Move Windows-authenticated logins by using T-SQL (SQL Server only) */

/*  The system catalog view [sys].server_principals contains the list of 
    Windows-authenticated logins (types ‘U’ for Windows user, and ‘G’ for Windows 
    group). The default_database_name and default_language_name are also provided 
    and you can script them with the login.
...
/*  Common security administration tasks\How to migrate SQL Server logins\Move 
    server roles using T-SQL (SQL Server only) */

/*  The following script retrieves server role membership via SQL Server catalog views.
    Here’s a sample script, note that it includes options to add logins to server roles. */

-- SERVER LEVEL ROLES
SELECT DISTINCT CONCAT('ALTER SERVER ROLE '
        ,  QUOTENAME([R].[NAME]) 
        , ' ADD MEMBER '
...
/*  Common security administration tasks\How to migrate SQL Server logins\Move server 
    permissions using T-SQL (SQL Server only) */
...
WHERE  [RM].[class_desc] = 'ENDPOINT' 
    AND [U].[is_fixed_role] = 0  -- NOTE: public is not considered a fixed role because you can grant it permissions
