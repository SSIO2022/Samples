...
/*
Consider the following opposing GRANT and DENY statements run from an administrative account on the WideWorldImporters sample database (which you can find here: https://docs.microsoft.com/en-us/sql/samples/wide-world-importers-oltp-install-configure).  
*/
...
/*
As a result, assuming the database user TestPermissions is only a member of this single role SalesSchemaRead, it would have permissions to execute SELECT statements on every object in the Sales schema, except for the Sales.SalesInvoice table. Let’s assume that no other permissions or role memberships have been granted to the database user TestPermissions. If this is run by user TestPermissions:
*/
